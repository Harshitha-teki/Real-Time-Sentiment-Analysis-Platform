...
Run this inside the backend container or from a machine that can reach the Postgres DB.
...
for i in range(6):
    ...
    cur.execute(
        "INSERT INTO sentiment_analysis (post_id, sentiment_label, confidence_score, emotion, model_name, created_at) VALUES (%s,%s,%s,%s,%s,%s)",
        (post_db_id, 'negative', 0.95, 'anger', 'seed-model', created_at)
    )
    conn.commit()
